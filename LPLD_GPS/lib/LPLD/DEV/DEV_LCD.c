...
extern const  unsigned char gImage_suqian[153600];
...
void LPLD_LCD_ShowStringno(uint16 Xpos, uint16 Ypos, uint8 *str,uint16 strColor );
...
uint16 LPLD_LCD_Init(void)
{
 ...
 // LPLD_LCD_INFO.Lcd_Id = dID;
  ...
  //  LPLD_LCD_WriteReg(0x0001,0x2B3F);    LPLD_LCD_Delayms(LCD_DELAY_TIME); 
    ...
    //  693F   6B3F   2B3F   293F
    LPLD_LCD_WriteReg(0x0001,0x293F);    LPLD_LCD_Delayms(LCD_DELAY_TIME);   
   ...
   // LPLD_LCD_WriteReg(0x0044,0xEF00);    LPLD_LCD_Delayms(LCD_DELAY_TIME);
   // LPLD_LCD_WriteReg(0x0045,0x0000);    LPLD_LCD_Delayms(LCD_DELAY_TIME); 
   // LPLD_LCD_WriteReg(0x0046,0x013F);    LPLD_LCD_Delayms(LCD_DELAY_TIME);
 ...
 //   LPLD_LCD_WriteReg(0x0025,0xe000);    LPLD_LCD_Delayms(LCD_DELAY_TIME);
  ...
  //  LPLD_LCD_WriteReg(0x004f,0);
  //  LPLD_LCD_WriteReg(0x004e,0);//设定显示窗口       
...
//不开窗写函数
void LPLD_LCD_ShowCharno( uint16 Xpos, uint16 Ypos, uint8 ASCI, uint16 charColor )
{
  uint16 i, j,coloer,te1,te2;
  uint8 tmp_char;
  for( i=0; i<16; i++ )
  {
    tmp_char = asc2_1608[ASCI-32][i];
    for( j=0; j<8; j++ )
    {
      ...
      else
      {
        te1=Xpos + j;
        te2 = Ypos + i;
        coloer=gImage_suqian[((Xpos + j)+(Ypos + i)*320)*2];
        coloer<<=8;
        coloer=coloer&0xff00;
        coloer |= gImage_suqian[((Xpos + j)+(Ypos + i)*320)*2+1];        
        LPLD_LCD_SetPoint( Xpos + j, Ypos + i, coloer );
      }
    }
  }
}
...
void LPLD_LCD_ShowStringno(uint16 Xpos, uint16 Ypos, uint8 *str,uint16 strColor )
{
  ...
  {
    TempChar = *str++;  
    LPLD_LCD_ShowCharno( Xpos, Ypos, TempChar, strColor );    
    if( Xpos <= MAX_X - 8 )
    {
      Xpos += 8;
    } 
